...
def transfer_money(tool_input: str):
    ...
    # result = {"status": "cancelled", "message": "Transaction cancelled by user."}

    if "confirmation" not in data:
        result = {
            "status": "confirm",
            "message": f"Do you really want to transfer {amount} dollars from {sender} to {receiver}?",
    ...
    try:
        ...
        result = {
            "status": "success",
            "message": f"Transfer successful. New balance: {sender} = {new_sender_balance}"
        }
